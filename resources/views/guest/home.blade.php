...
<div class="container mx-auto p-4">
    <div class="mb-8 md:mt-8 md:mb-24">
        <h2 class="text-center mb-8 text-red-600 font-bold text-xl">FEATURED BOOKS</h2>
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
            @foreach($featuredBooks as $featuredBook)
                <a href="{{ route('book.show', $featuredBook->book->id) }}">
                    <div class="book bg-white p-4 rounded-lg shadow-md">
    ...
    <div class="grid grid-cols-1 md:flex md:flex-wrap gap-4">
        <div class="author-info p-5 rounded-lg md:flex-1 md:w-1/3">
            <h2 class="text-red-600 font-bold text-xl">FEATURED AUTHOR</h2>
        ...
        <div class="books flex-1 md:w-1/3 grid grid-cols-4 md:grid-cols-2 gap-4">
            @foreach($featuredAuthor->author->books as $book)
                <a href="{{ route('book.show', $book->id) }}">
                    <div class="book shadow-md">
                        <img class="w-full h-48 object-cover rounded-lg" src="{{ $book->cover_img }}" alt="{{ $book->name }}">
